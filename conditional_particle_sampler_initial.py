...
def particle_filter(T,N,v,w,X,Y):
    ...
    for i in range(1,T+1):
        ...
        weights[i] = weight_n
        particles_resampled, A[i-1] = resample(particles.transpose(),weight_n,N)
...
def marginal_likelihood(T,N,v,w,X,Y):
    ...
    particles, weights, B = particle_filter(T,N,v,w,X,Y)
    log_ml = 0
    for i in range(T):
...
def particle_sample(T,N,v,w,X,Y):
    log_ml, particles, B = marginal_likelihood(T,N,v,w,X,Y)
